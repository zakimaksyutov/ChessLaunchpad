...
- `DataAccessProxyLayer` exposes exactly two methods: `retrieveRepertoireData` and `storeRepertoireData`. On construction it captures `(data, etag)` from `SessionStore.getSnapshot()`. `retrieveRepertoireData()` returns SessionStore's current cached data and **does NOT touch the proxy's locked etag** — pulling a fresher etag forward on read would let a `retrieve → user-pauses → intervening writer → store` sequence silently overwrite the intervening writer's changes, bypassing the conflict modal that the design promises is the only 412 recovery path. `storeRepertoireData(data)` calls `SessionStore.save(data, this.etag)` and updates `this.etag` from the response (so multi-save sequences on the same proxy work). The proxy's etag is therefore set once at construction and only ever advances via a successful save. It does not implement the full `IDataAccessLayer` interface — account ops and ETag-bypass remain on the one-shot DAL / SessionStore.importBlob respectively.
...
| Method | Behavior |
| --- | --- |
| `retrieveRepertoireData()` | Returns `SessionStore.getSnapshot().data` (cached, no network); does NOT touch `this.etag` (locked-etag invariant — see proxy section above) |
