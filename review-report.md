...
| M3 | Duplication | ✅ **FIXED — Card-state classification duplicated.** Replaced raw numeric literals (`0/1/2/3`) with `State` enum from ts-fsrs. Removed per-card closure; `isDue` is now a plain boolean. | `DashboardPage.tsx:12-36` vs `ReviewQueue.ts` |
